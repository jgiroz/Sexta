...
export default function Levantamientos() {
  ...
  const cargar = useCallback(async () => {
    ...
    let query = supabase
      .from('levantamientos')
      .select(
        'id, titulo, categoria, subcategoria, estado, prioridad, ubicacion, foto_url, creado_at, carro_id, asignado_a, carros(codigo)'
      )
              ...
              {levantamientos.map((l) => (
                <tr key={l.id}>
                  ...
                  <td>
                    <select
                      value={l.estado}
                      onChange={(e) => actualizarEstado(l.id, e.target.value)}
                    >
                      {ESTADOS.map((e) => (
